...
import { fetchWAQICity } from "../../lib/api";
...
function generateMockTrend(baseAqi: number) {
...
interface CityAQI {
  city: string; aqi: number; pm25: number; pm10: number;
  dominantPollutant: string; timestamp: string; live: boolean;
}
...
function MiniGauge({ value, color }: { value: number; color: string }) {
  const size = 48, sw = 4, r = (size - sw) / 2, c = 2 * Math.PI * r;
  return (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadLiveData() {
      try {
        const results = await Promise.allSettled(
          GHANA_CITIES.map(async (city) => {
            ...
            if (reading) return { city: city.city, aqi: reading.aqi, pm25: reading.pm25, pm10: reading.pm10, dominantPollutant: reading.dominantPollutant, timestamp: reading.timestamp, live: true };
      ...
      } catch { setError("Live API unavailable — showing baseline estimates."); }
    }
    loadLiveData();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* Top row: gauge + stats */}
      <div className="dash-top" style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 16, alignItems: "stretch" }}>
        <Tile style={{ padding: 24, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontSize: 10, color: "#8d8d8d", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>National Average</span>
        ...
        <div className="dash-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          {[
            ...
            <Tile key={stat.label} style={{ padding: 16, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <p style={{ fontSize: 11, color: "#8d8d8d", marginBottom: 4 }}>{stat.label}</p>
              <p style={{ fontSize: "1.375rem", fontWeight: 600, color: "#f4f4f4", fontFamily: "'IBM Plex Mono', monospace" }}>{stat.value}</p>
              {stat.sub && <p style={{ fontSize: 11, color: "#c6c6c6", marginTop: 2 }}>{stat.sub}</p>}
            </Tile>
          ))}
        </div>
      </div>
      ...
      {/* Map + sidebar */}
      <div className="dash-map" style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16 }}>
        <PollutionMap cities={mapCities} />
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Tile style={{ padding: 20 }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: "#c6c6c6", marginBottom: 12 }}>Health Advisory</h3>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: getAQIColor(avgAqi) }} />
              <span style={{ fontSize: 12, color: "#a8a8a8" }}>National: {getAQICategory(avgAqi).label}</span>
            </div>
            <p style={{ fontSize: 13, color: "#c6c6c6", lineHeight: 1.6, marginBottom: 12 }}>{getAQICategory(avgAqi).advice}</p>
            {worstCity.aqi > avgAqi && (
              <div style={{ borderTop: "1px solid #393939", paddingTop: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#a8a8a8" }}>Alert: {worstCity.city}</span>
                </div>
                <p style={{ fontSize: 13, color: "#c6c6c6", lineHeight: 1.6 }}>{getAQICategory(worstCity.aqi).advice}</p>
              </div>
            )}
          </Tile>
          <Tile style={{ padding: 20, flex: 1 }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: "#c6c6c6", marginBottom: 12 }}>AQI Scale</h3>
            {[
              { range: "0–50", label: "Good", color: "#4ade80" },
              { range: "51–100", label: "Moderate", color: "#facc15" },
              { range: "101–150", label: "USG", color: "#fb923c" },
              ...
              <div key={level.range} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 11, color: "#8d8d8d", width: 44, fontFamily: "monospace" }}>{level.range}</span>
                <span style={{ fontSize: 12, color: "#c6c6c6" }}>{level.label}</span>
              </div>
            ))}
          </Tile>
        </div>
      ...
      {/* City cards */}
      <div>
        <h2 style={{ fontSize: "1.125rem", fontWeight: 600, color: "#f4f4f4", marginBottom: 16 }}>City Air Quality</h2>
        <div className="dash-cards" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {cityData.map((city) => {
              ...
              <Tile key={city.city} style={{ padding: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 12 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: "#6f6f6f" }}>Updated {new Date(city.timestamp).toLocaleTimeString("en-GH", { hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  <div style={{ position: "relative" }}>
                    <MiniGauge value={city.aqi} color={cat.color} />
                    <span style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: cat.color, fontFamily: "'IBM Plex Mono', monospace" }}>{city.aqi}</span>
                  </div>
                </div>
                <span style={{ display: "inline-block", fontSize: 11, fontWeight: 500, padding: "2px 8px", borderRadius: 4, color: cat.color, background: cat.color + "15", marginBottom: 12 }}>{cat.label}</span>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                  <div>
                    <span style={{ fontSize: 10, color: "#6f6f6f" }}>PM2.5</span>
                    <p style={{ fontSize: 14, color: "#c6c6c6", fontFamily: "'IBM Plex Mono', monospace" }}>{city.pm25.toFixed(1)}</p>
                  </div>
                  <div>
                    <span style={{ fontSize: 10, color: "#6f6f6f" }}>PM10</span>
                    <p style={{ fontSize: 14, color: "#c6c6c6", fontFamily: "'IBM Plex Mono', monospace" }}>{city.pm10.toFixed(1)}</p>
                  </div>
                  <div>
                    <span style={{ fontSize: 10, color: "#6f6f6f" }}>Main</span>
                    <p style={{ fontSize: 12, color: "#c6c6c6", fontFamily: "'IBM Plex Mono', monospace", textTransform: "uppercase" }}>{city.dominantPollutant}</p>
                  </div>
                </div>
                <p style={{ fontSize: 12, color: "#8d8d8d", lineHeight: 1.5 }}>{cat.advice}</p>
              </Tile>
            );
          })}
        </div>
      ...
      {/* Heatmap */}
      <CityHeatmap cities={cityData.map((c) => ({ city: c.city, baseAqi: c.aqi }))} />

      {/* Trend charts */}
      <div className="dash-charts" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
